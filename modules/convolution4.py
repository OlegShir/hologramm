...
class Convolution():
    ...
    def azimuth_convolution_ChKP(self, 
        ...
        R_start = self.R + N_otst_y*self.dnr
        ...
        for i in range(Nbl_r):
            if Nbl_r == 1:
                ...
                R_centr_bl = R_start + ((i+1) * Ndr - Ndr / 2) * self.dnr  
    ...
    def save_RLI_PIL(self, RLI):
        ...
        Coef_1_r = 0.25 / self.dnr  # Коэффициент переквантования РГГ по дальности
        Coef_1_x = 0.25 / self.dx  # Коэффициент переквантования РГГ по азимуту
        # Вычисление амплитуды (модуля) 
        amplitude_values = np.abs(RLI)

  
        normalized_values = (amplitude_values / np.max(amplitude_values)) * 255
        ...
        image = image.resize((int(image.width*Coef_1_r), int(image.height*Coef_1_x)), Image.HAMMING)
        h = image.histogram()
        print(h[0:100])
...
if __name__ == '__main__':
    ...
    param_RSA = [10944+64, 40000, 400e6, 0.0351, 485.692e-6, 300e6, 10e-6, 108, 0.23, 2250]
    ...
    sf.azimuth_convolution_ChKP(ROI=[20000, 2000, 20000, 4000])
